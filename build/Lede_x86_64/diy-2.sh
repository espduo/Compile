...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.1.111'" $ZZZ              # IPv4 地址(openwrt后台地址)
sed -i "/uci commit network/i\uci set network.lan.netmask='255.255.255.0'" $ZZZ           # IPv4 子网掩码
sed -i "/uci commit network/i\uci set network.lan.gateway='192.168.1.1'" $ZZZ             # IPv4 网关
sed -i "/uci commit network/i\uci set network.lan.broadcast='192.168.1.255'" $ZZZ         # IPv4 广播
sed -i "/uci commit network/i\uci set network.lan.dns='202.96.64.68'" $ZZZ   # DNS(多个DNS要用空格分开)
...
sed -i "s/OpenWrt /espduo build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ        # 增加自己个性名称281677160
